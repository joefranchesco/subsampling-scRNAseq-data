...
def mainsubsample(dfs, readnumber, cellnumber, outputdfNames,outputDIR):
...
#so then I'd automatically end up with 1/3 or 1/5 of the max number of cells available at that specified depth for the 4 samples?
	...
	if mincellsleft<cellnumber:
		print('not enough cells!!! no!!!')
#think I don't want the above because I don't want to always subsample to the max cell number possible of the 4 samples
#likely want to subsample much smaller than this, and want explicit control of the same number to get to.
#	dfs3=[]
	else:
		for dataframe,outName in zip(dfs2,outputdfNames):
			newdf = subsampletocellnumber(cellnumber, dataframe)
...
#       try :
	...
	nameEnd='_'+str(ReadNumber)+'reads_'+str(Cellnumber)+'cells_'+str(SubsampleBarcode)+'.pkl'
	...
	SubsampledMatrix=mainsubsample(FullDenseMatrixList, ReadNumber, Cellnumber, outputdfNameList, Directory)
